"""This script is used to find points satisfying the following requirements:
1, points with large RMSE for CCDC fit, but not giant RMSE indicating image noise from detections.
2, the difference between large and giant is identified by quantile (90%).
...
study_area=["Amazon_Forest","Borneo_Indonesia","Cerrado_Brazil","Southeastern_US"]#"Americas"
#["Amazon_Forest","Borneo_Indonesia","Cerrado_Brazil","Southeastern_US"]
...
data=pd.concat(data)
data.to_csv(os.path.join(script_directory,"data_R_0.003_unstable.csv"),index=False)

data_dir=os.path.join(script_directory,"data_R_0.003_unstable.csv")
...
Data.to_csv(os.path.join(script_directory,"rmse_R_0.003_unstable.csv"),index=False)
